...
GROUPS = {
    "Volumes and sourcing": ["notified", "metered", "pv_buy_notified", "bl_buy_notified", "spot_buy_notified", "buy_notified", "pv_buy_metered",
                             "bl_buy_metered", "spot_settlement", "buy_metered", "share_pv", "share_bl", "share_spot"],
    "Costs and prices": ["price_pv_budget", "price_bl_budget", "price_spot", "price_total_budget", "cost_pv_budget", "cost_bl_budget", "cost_spot", "cost_budget",
    ...
    "Retail revenue and margins": ["sell_price", "revenue", "gm1_budget", "gm1_budget_pct", "gm1_budget_specific", "gm1_forecast", "gm1_forecast_pct",
    ...
    "Wholesale resell": ["rs_volume", "rs_volume_mw", "rs_cost_forecast", "rs_cost_budget", "rs_revenue", "rs_source_imb", "rs_gm1_budget",
    ...
    "Totals": ["t_buy_notified", "t_buy_metered", "t_cost_budget", "t_cost_forecast", "t_revenue", "t_imb", "t_gm2_budget", "t_gm2_budget_pct",
               "t_gm2_budget_specific", "t_gm2_forecast", "t_gm2_forecast_pct", "t_gm2_forecast_specific"],
    "Cost to serve and financing": ["passthrough_revenue", "passthrough_cost", "reserve", "opex", "variable_opex", "offtaker_bgl_fees",
    ...
    "Net margin and tax": ["nm_budget", "nm_budget_pct", "nm_budget_specific", "nm_forecast", "nm_forecast_pct", "nm_forecast_specific", "cit_budget",
    ...
    "Green certificates (memo)": ["gc_count", "gc_spot", "gc_bilateral", "gc_value", "gc_unit"],
...
def render() -> None:
    ...
    p = r.params
    B.kpi_row([
        ("Total GM2 forecasted", P.y("t_gm2_forecast"), "EUR", f"{B.num(P.y('t_gm2_forecast_specific'), 2)} EUR/MWh bought"),
        ("Cost to serve", P.y("opex") + P.y("variable_opex") + P.y("offtaker_bgl_fees") + P.y("market_bgl_fees") + P.y("interest"), "EUR",
         "OPEX + variable OPEX + BGL fees + interest"),
        ("NM pre-tax forecasted", P.y("nm_forecast"), "EUR", f"{B.pct(P.y('nm_forecast_pct'))} of revenue · {B.num(P.y('nm_forecast_specific'), 2)} EUR/MWh"),
        ("CIT forecasted", P.y("cit_forecast"), "EUR", f"Rate {B.pct(p.general.cit_rate, 0)} on cumulative year-to-date NM, floored at 0, quarterly"),
    ])

    st.markdown("## Margin build-up by month · forecast case · EUR")
    m = {"GM1": P.m("gm1_forecast"), "GM2": P.m("gm2_forecast"), "Resell GM2": P.m("rs_gm2_forecast"), "NM pre-tax": P.m("nm_forecast")}
    ...
    which = st.multiselect("Blocks", list(GROUPS), default=["Retail revenue and margins", "Totals", "Cost to serve and financing", "Net margin and tax"])
    for name in which:
    ...
    o = p.offtaker(code)
    B.kpi_row([
        ("Metered volume", T.y("metered"), "MWh", f"Contract price {B.num(o.contract_price_eur_per_mwh, 2)} EUR/MWh"),
        ("GM2 forecasted", T.y("gm2_forecast"), "EUR", f"{B.num(T.y('gm2_forecast_specific'), 2)} EUR/MWh"),
        ("Retail NM pre-tax forecasted", T.y("retail_nm_forecast"), "EUR", f"{B.num(T.y('retail_nm_forecast_specific'), 2)} EUR/MWh"),
        ("Own guarantee (peak)", float(np.max(T.m("own_guarantee"))) if "own_guarantee" in T else 0.0, "EUR", f"BGL fee {B.num(T.y('own_bgl_fee'), 0)} EUR"),
    ])
